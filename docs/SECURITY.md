...
Nyth Router is designed for private deployments. By default, the server binds to `127.0.0.1`, does not include telemetry, and stores secrets in an encrypted SQLite-backed vault.

## Threat model

Nyth Router protects against:
...
Nyth Router does **not** protect against:
...
If you need stronger guarantees, run Nyth Router inside a hardened VM / container and store the master key in an external secret manager.
...
- **Master key**: 32 random bytes (hex). Sourced from `NYTH_MASTER_KEY` if set; otherwise auto-generated to `server/data/master.key` on first boot. The file is created with mode `0600`.
...
- Nyth Router refuses to seed dashboard access with an empty or short `NYTH_PASSWORD`; use at least 15 characters.
...
`Settings → Retention` controls how many days of `request_logs` and `fallback_events` Nyth Router keeps. Older rows are purged on a daily sweep. Set to `0` to disable retention sweeps and keep logs forever.

## Reset / wipe

- **Settings → Danger zone → Reset database** deletes `server/data/nyth.db` and re-runs migrations.
- Manually: stop the server, delete `server/data/nyth.db` and `server/data/master.key`, restart.
