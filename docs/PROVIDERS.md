...
Bigliner ships with a static **provider registry** that catalogs 100+ AI providers and 175+ models. The registry is metadata-first: every provider has enough metadata to render a card, badge, and model list in the dashboard, but only a few have full forwarding adapters today.

## Statuses

| Status            | Meaning |
|-------------------|---------|
| `implemented`     | Bigliner can forward live `/v1/chat/completions` traffic to this provider using its adapter. |
...
- `openai-compatible` — exposes the OpenAI Chat Completions schema. Works through Bigliner's OpenAI adapter directly.
- `anthropic-compatible` — exposes Anthropic's `messages` API. Works through Bigliner's Anthropic adapter (translates messages, system prompt, and response).
- `native` — provider-specific API. Not yet routable through Bigliner without writing a custom adapter; still listed in the registry for visibility.
